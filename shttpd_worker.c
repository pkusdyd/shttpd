...
static void do_work(struct worker_ctl *wctl)
{
	...
	for(;retval > 0;)
	{
		...
		/*设置超时*/
		tv.tv_sec = 10;//conf_para.TimeOut;
		tv.tv_usec = 0;

		/*超时读数据*/
		DBGPRINT("in do_work before select, fd:%d\n", fd);
		retval = select(fd + 1, &rfds, NULL, NULL, &tv);
		DBGPRINT("in do_work after select, fd:%d, retval:%d\n", fd, retval);
		switch(retval)
		{
			case -1:/*错误*/
				close(fd);
			...
			case 0:/*超时*/
				...
				break;
			default:
				printf("select retval:%d, dreq_len:%d\n",retval, sizeof(wctl->conn.dreq));
...
int Worker_ScheduleRun(int ss)
{
	...
	for(;SCHEDULESTATUS== STATUS_RUNNING;)
	{
		...
		/*超时读数据*/
		DBGPRINT("in main before select, ss:%d\n", ss);
		retval = select(ss + 1, &rfds, NULL, NULL, &tv);
		DBGPRINT("in main after select, ss:%d, retval:%d\n", ss, retval);
		switch(retval)
		{
			case -1:/*错误*/
			case 0:/*超时*/
